...
if (isset($_GET['ReportRange'])) {
	...
	elseif ($_GET['ReportRange'] == "custom") {
		...
		if (isset($_GET['startdate'])) {
			if ($_GET['startdate'] == "") {
				echo "<font color=\"red\"><h1>Start date is missing!</h1></font></br>\n";
				echo "When setting custom report period, start date is required</br>\n";
				exit;
			}
		}
		...
		if ($starttime > $endtime) {
			echo "<font color=\"red\"><h1>Start date need to be before tomorrow or end date!</h1></font></br>\n";
			exit;
...
//	$reportname=sanitiseString($name);
	$reportname=str_replace([" ", "/", "."], "_",$name);
        $reportname=str_replace([".JDADELIVERS.COM", ".jdadelivers.com"], "",$name);
	CreatePDF($hosts);
}
...
//	$reportname=sanitiseString($name);
	$reportname=str_replace(["CUSTOMER/", "TYPE/", "PRODUCT/", "DC/", "MOM/"], "",$name);
        $reportname=str_replace([" ", "/", "."], "_",$name);
	CreatePDF($hosts);
}
else {
...
$pdf->ezText("Utilization Report",30,array('justification'=>'centre'));
...
$pdf->ezText("",10,array('justification'=>'centre'));
$pdf->ezText(date('l jS \of F Y \a\t H:i'),16,array('justification'=>'centre'));
...
$pdf->ezSetDy(-450);
$pdf->ezText("Report start  : $report_start",12,array('justification'=>'right'));
$pdf->ezText("Report end   : $report_end",12,array('justification'=>'right'));
$pdf->ezText("Timezone : $timezone (UTC)  ",12,array('justification'=>'right'));
$pdf->selectFont($mainFont);

// modified to use the local file if it can
if (file_exists($pdf_logo)){
  //$pdf->addPngFromFile($pdf_logo,199,$pdf->y-375,200,0);
  $pdf->addPngFromFile($pdf_logo,50,$pdf->y,150,0);
  if ($debug) {
